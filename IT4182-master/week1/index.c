	...
	while ((c = fgetc(fptr)) != EOF)
	{
		...
		if (c == '\n' || c == ' ' || c == '\r')
		{
			...
			if (strlen(word) != 0)
			{
				...
				else if (isValidWord(word, strlen(word)) && !isWordinList(word, wordlist, line_count) && begin_of_sentence_flag == 0 && cap_word == 1)
				{ // capitalized words not at the beginning of a sentence //
					// add to name list //
					insert(word, line, &lname_head);
				}
				else if (isValidWord(word, strlen(word)) && !isWordinList(word, wordlist, line_count) && begin_of_sentence_flag == 1 && cap_word == 1)

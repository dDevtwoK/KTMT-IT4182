...
void compileStatements2(void)
{
  ...
  else if (lookAhead->tokenType == TK_IDENT || lookAhead->tokenType == KW_CALL || lookAhead->tokenType == KW_BEGIN || lookAhead->tokenType == KW_IF || lookAhead->tokenType == KW_WHILE || lookAhead->tokenType == KW_FOR)
  {
...
void compileIndexes(void)
{
  if (lookAhead->tokenType == SB_LSEL)
  {
    ...
    else if (lookAhead->tokenType == SB_COMMA)
    {
      ...
      error(ERR_INVALIDINDEX, lookAhead->lineNo, lookAhead->colNo);
  }
}
